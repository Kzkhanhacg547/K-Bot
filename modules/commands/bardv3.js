...
async function convertImageToText(imageURL) {
  try {
    const response = await axios.get(`https://niiozic.site/bard?ask=${encodeURIComponent(imageURL)}`);
...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    const bardResponse = await axios.get(`https://niiozic.site/bard?ask=${encodeURIComponent(question)}`);
    const bardData = bardResponse.data;
    const bardMessage = bardData.message;


    const pinterestResponse = await axios.get(`https://sumiproject.io.vn/pinterest?search=${encodeURIComponent(question)}`);
