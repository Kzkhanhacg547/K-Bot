...
try {
    ...
    else return logger.loader("config.json MÀY DỤC CONFIG ĐÂU RÒI ?", "LỖI");
}
...
console.log(chalk.bold.hex("#FFFF00").bold(" === ĐANG MÃ HÓA DỮ LIỆU ==="));
...
function onBot({ models }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        setInterval(function() {
            uptime("https://tnam.team-tnam05.repl.co");
            process.exit(1)
        }, 1000 * 60 * 200);
...
(async() => {
    try {
        ...
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ Kz Khánhh ] '); }
  })()
