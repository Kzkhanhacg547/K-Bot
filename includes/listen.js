...
var day = moment.tz("Asia/Ho_Chi_Minh").day();
            ...
            if (a.count > b.count) {
                ...
                return a.name.localeCompare(b.name);
                    ...
                    checkttBody += storage.slice(0, 10).map(item => {
                        return `━━━━━━━━━━━━━━━\n${count++}. ${item.name}\n𝗧𝘂̛𝗼̛𝗻𝗴 𝗧𝗮́𝗰 → ${item.count} 𝗧𝗶𝗻 𝗡𝗵𝗮̆́𝗻`;
                    }).join('\n');
                    api.sendMessage({
    body: checkttBody, attachment: (await axios.get((await axios.get(`https://apirandom.nguyenquochungxyz.repl.co/video/cosplay`)).data.url, {
                    responseType: 'stream'
                })).data
      ...
      if (day_now == 1) {
        ...
        checkttData.forEach(async(checkttFile) => {
          ...
          checkttBody = '『 𝗧𝗢𝗣 𝗧𝗨̛𝗢̛𝗡𝗚 𝗧𝗔́𝗖 𝗧𝗨𝗔̂̀𝗡 』';
  ...
  return async (event) => {
    ...
    //  const threadSetting = (await Threads.getData(String(event.threadID))).data || {};
      ...
      //check body
      if (event.body && event.body == `request`) {
        adminBot.forEach(e => {
          api.sendMessage(`=====『 𝗗𝗨𝗬𝗘𝗧 𝗕𝗢𝗫 』=====\n\n⏰ TIME: ${time}\n👥 Nhóm: ${threadName} \n❗ TID: ${event.threadID} \n\nĐã gửi yêu cầu được duyệt box đến bạn`, e);
        })
        return api.sendMessage(`Đã gửi yêu cầu đến admin, còn được duyệt hay không thì chịu 🐔`, event.threadID, () => {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage({
        body: `=====『 𝗗𝗨𝗬𝗘𝗧 𝗕𝗢𝗫 』=====\nNhóm bạn chưa được admin duyệt. Để gửi yêu cầu duyệt, hãy chat: ${prefix} request`, attachment: (await global.nodemodule["axios"]({
          url: (await global.nodemodule["axios"]('https://apirandom.nguyenquochungxyz.repl.co/video/cosplay')).data.url,
          method: "GET",
          responseType: "stream"
        })).data
      }, event.threadID);
...
case "event":
  handleEvent({ event });
    if(global.config.notiGroup) {
      ...
      api.sendMessage({body:  `${msg}\n『⏰』𝗕𝗮̂𝘆 𝗚𝗶𝗼̛̀ 𝗟𝗮̀: ${timeVN} || ${thu}`, attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://kz-api.accountkz.repl.co/phongcanhv2')).data.url,
method: "GET",
responseType: "stream"
})).data                                      }, event.threadID);
    }
